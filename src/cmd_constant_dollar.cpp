...
swr::cli::command_schema constant_dollar_schema() {
    ...
    s.flags = {
        {"withdrawal-rate",    "w",  FlagGroup::REQUIRED, FlagKind::VALUE,    "Withdrawal rate (percent of initial portfolio)", ""},
        ...
        {"target-success",     "t",  FlagGroup::COMMON,   FlagKind::VALUE,    "If > 0, emit pass/fail vs this target (percent)", "0"},
...
int constant_dollar(const std::vector<std::string>& args) {
    ...
    try {
        ...
        sc.wr       = std::stof(swr::cli::get_value(p, schema, "withdrawal-rate"));
    ...
    float target_success = std::stof(swr::cli::get_value(p, schema, "target-success"));
