...
template <size_t N>
bool monthly_rebalance(const swr::scenario& scenario, std::array<float, N>& current_values) {
...
template <size_t N>
bool yearly_rebalance(const swr::scenario& scenario, std::array<float, N>& current_values) {
    // Nothing to rebalance if we have a single asset
    if constexpr (N == 1) {
        return true;
    }
...
template <size_t N>
bool pay_fees(const swr::scenario& scenario, std::array<float, N>& current_values) {
    // Simulate TER
    if (scenario.fees > 0.0f) {
        ...
        if (scenario.is_failure(current_value(current_values))) {
            return false;
        }
    }
        ...
        for (size_t current_month = 1; current_month <= 12; ++current_month) {
            ...
            for (size_t y = current_year; y <= end_year; ++y) {
                ...
                for (m = (y == current_year ? current_month : 1); !failure && m <= (y == end_year ? end_month : 12); ++m, ++context.months) {
                    ...
                    step([&]() { return !scenario.is_failure(current_value(current_values)); });

                    // Monthly Rebalance
                    step([&]() { return monthly_rebalance(scenario, current_values); });

                    // Simulate TER
                    step([&]() { return pay_fees(scenario, current_values); });
                ...
                step([&]() { return yearly_rebalance(scenario, current_values); });
