...
//
// `dynamic_success` command: given a budget, return the success probability.
//=======================================================================
...
swr::cli::command_schema dynamic_success_schema() {
    using namespace swr::cli;
    command_schema s;
    s.command_name = "dynamic-success";
    s.one_line_description =
        "Given my balance and a planned budget, how likely is that budget to\n"
        "  survive my remaining years? Runs one simulation at the given budget\n"
        "  and returns the historical success probability.";
    s.example_invocation =
        "drawdown dynamic-success -b 850000 -a 67 -e 92 -bd 50000 "
        "-p \"us_stocks:60;us_bonds:40;\"\n"
        "  drawdown dynamic-success --balance 850000 --current-age 67 "
    ...
    s.flags = {
        ...
        {"budget",             "bd", FlagGroup::REQUIRED, FlagKind::VALUE,    "Annual planned spending budget (dollars)", ""},
        ...
        {"prior-amount",       "pa", FlagGroup::COMMON,   FlagKind::VALUE,    "Last year's spending budget (enables signal field)", "0"},
        {"target-success",     "t",  FlagGroup::COMMON,   FlagKind::VALUE,    "Comparison target success rate (percent)", "80"},
        ...
        {"solver-tolerance",   "st", FlagGroup::ADVANCED, FlagKind::VALUE,    "Comparison binary-search stopping tolerance (dollars)", "1"},
    };
    s.mutually_exclusive.push_back({"json", "csv"});
    return s;
}

// Returns a one-line verdict based on probability vs comparison target.
std::string make_verdict(float probability, float target) {
    if (probability >= target) {
        return "meets target — budget is sustainable";
    } else if (probability >= 0.9f * target) {
        return "just under target — borderline";
    } else {
        return "below " + std::to_string((int)target) + ".0% target — budget is risky";
    }
}
...
int dynamic_success(const std::vector<std::string>& args) {
    auto schema = dynamic_success_schema();
    ...
    try {
        ...
        swr::output::emit_error(std::cerr, "dynamic-success", e.what(), mode);
        return 1;
    }
    ...
    // Build evaluate_input from parsed flags.
    ...
    try {
        ...
        in.budget =
            std::stof(swr::cli::get_value(p, schema, "budget"));
        in.portfolio =
        ...
        if (ssa_income > 0.0f) {
            if (ssa_age == 0) {
                swr::output::emit_error(std::cerr, "dynamic-success",
        ...
        in.prior_year_amount = std::stof(swr::cli::get_value(p, schema, "prior-amount"));
    ...
    // Evaluate. Collect per-path detail when CSV output is requested so the
    // rows correspond to the user's exact budget
    bool collect_csv = (mode == swr::output::Mode::CSV);
    auto r = swr::evaluate(in, collect_csv);
    if (r.error) {
        swr::output::emit_error(std::cerr, "dynamic-success", r.message, mode);
        return 1;
    }

    // Build report and emit.
    swr::output::report rep;
    rep.command_name = "dynamic-success";
    {
        ...
        sec.fields.push_back({"Annual budget", (double)in.budget,
                              swr::output::field::Hint::DOLLARS});
        sec.fields.push_back({"Comparison target",
                              (double)in.comparison_target_success,
                              swr::output::field::Hint::PERCENT});
        {
            std::stringstream port_ss;
            for (auto& alloc : in.portfolio)
                port_ss << alloc.asset << ":" << alloc.allocation << ";";
            sec.fields.push_back({"Portfolio", port_ss.str(),
                                  swr::output::field::Hint::NONE});
        }
        sec.fields.push_back({"Inflation series", in.inflation,
                              swr::output::field::Hint::NONE});
        {
        ...
        sec.fields.push_back({"Withdraw frequency", freq_label,
                              swr::output::field::Hint::NONE});
        sec.fields.push_back({"Fees (TER)", (double)(in.fees * 100.0),
                              swr::output::field::Hint::PERCENT});
    ...
    }
    {
        ...
        sec.name  = "results";
        sec.title = "Results";
        sec.fields.push_back({"Probability of success",
                              (double)r.probability_of_success,
                              swr::output::field::Hint::PERCENT});
        sec.fields.push_back({"Verdict",
                              make_verdict(r.probability_of_success,
                                           in.comparison_target_success),
                              swr::output::field::Hint::NONE});
        if (r.comparison_supported) {
            sec.fields.push_back({"Max budget at " +
                                  std::to_string((int)in.comparison_target_success) +
                                  ".0%",
                                  (double)r.comparison_max_budget,
                                  swr::output::field::Hint::DOLLARS});
        } else {
            sec.fields.push_back({"Max budget (comparison)",
                                  std::string("not supported at this horizon"),
                                  swr::output::field::Hint::NONE});
        }
    ...
    if (mode == swr::output::Mode::CSV) {
        ...
        pre << "command: dynamic-success, budget: " << (int)in.budget;
